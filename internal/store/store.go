...
CREATE INDEX IF NOT EXISTS knowledge_chunks_agent_idx ON %s.knowledge_chunks (tenant_id, agent_id);`,
	...
	if err := s.ensureCommercialSchema(ctx); err != nil {
		return err
	}
	if err := s.ensurePromotionSchema(ctx); err != nil {
		return err
	}
	if err := s.ensureEmbedSchema(ctx); err != nil {
		return err
	}
	if err := s.ensureTenantAISchema(ctx); err != nil {
		return err
	}
	...
	if err := s.ensureThemeSchema(ctx); err != nil {
		return err
	}
	...
	if err := s.ensureTicketsSchema(ctx); err != nil {
		return err
	}
	if err := s.ensureUsageSchema(ctx); err != nil {
		return err
	}
	if err := s.ensureLeadsSchema(ctx); err != nil {
		return err
	}
